...
- Preset constants are fixed by the spec and must be used verbatim: `smooth` = stiffness 1080 / damping 65, `snappy` = stiffness 1400 / damping 43, `bouncy` = stiffness 5200 / damping 35. Mass is 1 throughout. Default preset is `snappy`. (Revised after Task 5 shipped: the original stiffness/damping pairs preserved the right damping ratios but settled in 658–1575ms, too slow for a fixed-duration CSS transition, so both were scaled up together to hold the ratio — and therefore the feel — fixed while compressing the duration to roughly 400ms. Revised again after release: holding every preset to roughly 400ms left them indistinguishable in a browser, so the constants are now `smooth` 2425/98, `snappy` 9375/116 and `bouncy` 2425/16, settling in 285ms, 120ms and 901ms. See the spec's preset table.)
- Preset names are exactly `smooth`, `snappy`, `bouncy`, `none`. The prop is named `animation` on every component.
- Presets vary physics only. Never vary travel distance, scale depth, or any other choreography between presets. (Superseded after release: this rule is what made the three presets indistinguishable, because overshoot is a proportion of the distance travelled and a fixed 4% press left `bouncy` bouncing across half a pixel. Press depth is now part of the preset — 0.96, 0.94, 0.88. What a gesture _is_ stays fixed per component; how far and how fast it goes belongs to the preset. See the spec's preset contract.)
